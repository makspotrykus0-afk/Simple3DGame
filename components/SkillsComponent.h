...
struct Skill {
    ...
    Skill() : type(SkillType::UNKNOWN), name("Unknown"), level(1), currentXP(0.0f), xpToNextLevel(100.0f), priority(0) {}
    Skill(SkillType t, int l, float xp)
        : type(t), name(skillTypeToString(t)), level(l), currentXP(xp), xpToNextLevel(100.0f * (l + 1)), priority(0) {}
};
...
public:
    ...
    std::type_index getComponentType() const override { return std::type_index(typeid(SkillsComponent)); }
