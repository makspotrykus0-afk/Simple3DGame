...
public:
    BuildingInstance(const std::string& blueprintId, const Vector3& position, float rotation)
        : GameEntity("Building_" + blueprintId), m_blueprintId(blueprintId), m_rotation(rotation), m_constructionProgress(0.0f), m_isBuilt(false), m_health(100.0f), m_position(position), m_maxHealth(100.0f), m_cachedBlueprint(nullptr) {
        // Initialize m_position in member initializer list to avoid ambiguity
    }
    ...
    void updateVisualStorage(const std::vector<StorageSlot>& storageSlots) {
        ...
        if (!m_visualSlots.empty()) {
            std::cout << "[DEBUG] First visual slot pos (local): " << m_visualSlots[0].localPosition.x << ", " << m_visualSlots[0].localPosition.y << ", " << m_visualSlots[0].localPosition.z << std::endl;
        }
    }
    ...
    BoundingBox getBoundingBox() const {
        ...
        return {
            {m_position.x - halfSize.x, m_position.y, m_position.z - halfSize.z},
            {m_position.x + halfSize.x, m_position.y + size.y, m_position.z + halfSize.z}
        };
    }
    ...
    void render() override {
        if (m_isBuilt) {
            DrawCube(m_position, 2.0f, 2.0f, 2.0f, GRAY);
            
            // Renderowanie zawartosci magazynu
            for (const auto& vSlot : m_visualSlots) {
                // Transformacja do swiata z uwzględnieniem rotacji
                Vector3 rotatedPos = Vector3RotateByAxisAngle(vSlot.localPosition, {0.0f, 1.0f, 0.0f}, m_rotation * DEG2RAD);
                Vector3 worldPos = Vector3Add(m_position, rotatedPos);
                
                Color color = BROWN;
                if (vSlot.type == Resources::ResourceType::Stone) color = GRAY;
                else if (vSlot.type == Resources::ResourceType::Metal) color = DARKGRAY;
                else if (vSlot.type == Resources::ResourceType::Gold) color = GOLD;
                else if (vSlot.type == Resources::ResourceType::Food) color = RED;
                
                // Rysuj sześcian uwzględniając rotację
                // Ponieważ DrawCube nie obsługuje rotacji, używamy DrawModel lub transformacji macierzy
                // Dla uproszczenia rysujemy sfery lub używamy rlgl push/pop matrix
                
                rlPushMatrix();
                rlTranslatef(worldPos.x, worldPos.y, worldPos.z);
                rlRotatef(m_rotation, 0.0f, 1.0f, 0.0f);
                // Since we are in already rotated/translated space, draw at origin
                DrawCube(Vector3{0,0,0}, vSlot.scale.x, vSlot.scale.y, vSlot.scale.z, color);
                DrawCubeWires(Vector3{0,0,0}, vSlot.scale.x, vSlot.scale.y, vSlot.scale.z, BLACK);
                rlPopMatrix();
            }
            
        } else {
            DrawCubeWires(m_position, 2.0f, 2.0f, 2.0f, YELLOW);
        }
    }
...
private:
    ...
    // Explicitly define m_position here to shadow whatever is in base or missing
    Vector3 m_position;
