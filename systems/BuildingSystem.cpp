...
BuildTask::BuildTask(BuildingBlueprint* blueprint, Vector3 position, GameEntity* builder, float rotation)
    : m_blueprint(blueprint), m_position(position), m_builder(builder), m_rotation(rotation),
      m_progress(0.0f), m_state(BuildState::PLANNING), m_startTime(0.0f), m_assignedWorkers(0)
{
...
void BuildTask::update(float deltaTime) {
    (void)deltaTime;
}

void BuildTask::cancel() {
    m_state = BuildState::CANCELLED;
}
...
void BuildTask::advanceConstruction(float amount) {
    if (m_state != BuildState::CONSTRUCTION && m_state != BuildState::PLANNING) return;
    
    if (!hasAllResources()) return;
    
    if (m_state == BuildState::PLANNING) m_state = BuildState::CONSTRUCTION;
...
BoundingBox BuildTask::getBoundingBox() const {
    ...
    return BoundingBox{
        Vector3Subtract(m_position, halfSize),
...
BuildingSystem::BuildingSystem() : GameSystem("BuildingSystem"), m_interactionSystem(nullptr), m_colony(nullptr), m_gridSize(1.0f), m_snapToGrid(true), m_isPlanning(false) {
...
void BuildingSystem::initialize() {
    ...
    if (m_storageSystem) {
        ...
        std::cerr << "BuildingSystem: Failed to connect to StorageSystem!" << std::endl;
    }
...
void BuildingSystem::ensureBlueprintExists(const std::string& blueprintId) {
    if (m_blueprints.find(blueprintId) != m_blueprints.end()) return;
    ...
    if (blueprintId.find("house_") == 0) {
        ...
        try {
            ...
            else if (size == 6) { width = 3; length = 2; }
            else if (size == 9) { width = 3; length = 3; }
            else {
                width = (int)std::sqrt(size);
                length = size / width;
            }
            
            std::string name = "House " + std::to_string(width) + "x" + std::to_string(length);
            auto house = std::make_unique<BuildingBlueprint>(blueprintId, name, BuildingCategory::RESIDENTIAL);
            ...
            std::cout << "BuildingSystem: Generated dynamic blueprint " << blueprintId << " with Bed" << std::endl;

        } catch (...) {
            std::cerr << "BuildingSystem: Failed to parse dynamic blueprint ID: " << blueprintId << std::endl;
        }
    }
}

void BuildingSystem::registerDefaultBlueprints() {
    auto wall = std::make_unique<BuildingBlueprint>("wall", "Wall", BuildingCategory::STRUCTURE);
    ...
    auto floor = std::make_unique<BuildingBlueprint>("floor", "Floor", BuildingCategory::STRUCTURE);
    ...
    auto door = std::make_unique<BuildingBlueprint>("door", "Door", BuildingCategory::STRUCTURE);
    ...
    auto bed = std::make_unique<BuildingBlueprint>("bed", "Bed", BuildingCategory::FURNITURE);
    ...
    auto stockpile = std::make_unique<BuildingBlueprint>("stockpile", "Stockpile", BuildingCategory::STORAGE);
    ...
    auto simpleStorage = std::make_unique<BuildingBlueprint>("simple_storage", "Simple Storage", BuildingCategory::STORAGE);
...
void BuildingSystem::registerBlueprint(std::unique_ptr<BuildingBlueprint> blueprint) {
    // Try loading model, but don't fail if missing (will use fallback)
    // Better error handling in loadModel would be ideal, but here we assume loadModel handles its own errors/defaults
    blueprint->loadModel(); 
...
bool BuildingSystem::canBuild(const std::string& blueprintId, Vector3 position) {
    const_cast<BuildingSystem*>(this)->ensureBlueprintExists(blueprintId);
    if (m_blueprints.find(blueprintId) == m_blueprints.end()) return false;
    return true;
}

BuildTask* BuildingSystem::startBuilding(const std::string& blueprintId, Vector3 position, GameEntity* builder, float rotation, bool forceNoSnap, bool ignoreCollision, bool instant, bool* outSuccess) {
    // Using forceNoSnap and ignoreCollision just to silence warnings for now
    (void)forceNoSnap;
    (void)ignoreCollision;

    std::cout << "BuildingSystem: startBuilding called for " << blueprintId << " (instant=" << instant << ")" << std::endl;
    
    ensureBlueprintExists(blueprintId);

    if (m_blueprints.find(blueprintId) == m_blueprints.end()) {
        std::cerr << "BuildingSystem: Blueprint not found: " << blueprintId << std::endl;
        if (outSuccess) *outSuccess = false;
        return nullptr;
    }

    BuildingBlueprint* bp = m_blueprints[blueprintId].get();
    std::cout << "BuildingSystem: Found blueprint " << blueprintId << ", components: " << bp->getComponents().size() << std::endl;
    
    if (instant) {
        auto building = std::make_unique<BuildingInstance>(blueprintId, position, rotation);
        ...
        if (m_storageSystem) {
            ...
            if (blueprintId == "simple_storage" || blueprintId == "stockpile" || bp->getCategory() == BuildingCategory::STORAGE) {
                std::string storageId = m_storageSystem->createStorage(StorageType::WAREHOUSE, "Colony");
                building->setStorageId(storageId);
                std::cout << "  Storage (Warehouse) created for instant building (" << blueprintId << "): " << storageId << std::endl;
            } else if (bp->getCategory() == BuildingCategory::RESIDENTIAL) {
                std::string storageId = m_storageSystem->createStorage(StorageType::CHEST, "Colony");
                building->setStorageId(storageId);
                std::cout << "  Storage (Chest) created for instant house: " << storageId << std::endl;
            }
        }

        if (blueprintId == "door") {
             auto doorObj = std::make_shared<Door>("Door", position, rotation, m_colony);
            ...
            for (const auto& comp : bp->getComponents()) {
                ...
                if (comp.blueprintId == "door") {
                    auto doorObj = std::make_shared<Door>("Door", compPos, compRot, m_colony);
        ...
        // We need to release the unique_ptr ownership before pushing to vector, or push directly
        // Since startBuilding returns a BuildTask* (or nullptr if instant), we don't need to return the building ptr.
        // However, m_buildings stores unique_ptr<BuildingInstance>.
        m_buildings.push_back(std::move(building));
        
        std::cout << "BuildingSystem: Building added to list. Total buildings: " << m_buildings.size() << std::endl;
...
void BuildingSystem::update(float dt) {
    ...
    if (timer > 1.0f) {
        timer = 0.0f;
        if (m_storageSystem) {
            std::cout << "[DEBUG] BuildingSystem::update: Checking storages for visualization update..." << std::endl;
            for (const auto& building : m_buildings) {
                // Check if it's a storage
                std::string bpId = building->getBlueprintId();
                std::string bpName = building->getBlueprint() ? building->getBlueprint()->getName() : "";
                
                // Jeśli budynek jest magazynem (nawet jeśli w trakcie budowy, jeśli ma już przypisane ID magazynu)
                // Sprawdzamy szerzej, czy to magazyn, na podstawie blueprintu lub kategorii
                bool isStorage = (bpId == "simple_storage" || bpId == "stockpile" ||
                                  bpName == "storehouse" || bpName == "Stockpile" || bpName == "Simple Storage");
                ...
                // Jeśli building jest w trakcie budowy, ale ma już storageId (np. instant build lub przypisane przy starcie)
                // to chcemy zaktualizować wizualizację.
                // Jeśli building jest 'under construction' (progress < 100), to normalnie może nie być w pełni funkcjonalny,
                // ale jeśli ma storageId, to znaczy że system magazynowy go obsługuje.
                
                if (isStorage) {
                    std::string storageId = building->getStorageId();
                    std::cout << "[DEBUG] Found storage building " << building->getBlueprintId() << " ID=" << storageId << std::endl;
                    ...
                    if (!storageId.empty()) {
                        ...
                        if (storage) {
                            ...
                            std::cout << "[DEBUG] Storage instance not found for ID=" << storageId << std::endl;
                        }
                    } else {
                        std::cout << "[DEBUG] Storage building has empty storageId" << std::endl;
                    }
                }
            }
        }
...
void BuildingSystem::completeBuilding(BuildTask* task) {
    auto building = std::make_unique<BuildingInstance>(task->getBlueprint()->getId(), task->getPosition(), task->getRotation());
    ...
    if (m_storageSystem) {
        ...
        if (bpId == "simple_storage" || task->getBlueprint()->getCategory() == BuildingCategory::STORAGE) {
            std::string storageId = m_storageSystem->createStorage(StorageType::WAREHOUSE, "Colony");
            building->setStorageId(storageId);
            std::cout << "  Storage (Warehouse) created for building (" << bpId << "): " << storageId << std::endl;
        } else if (task->getBlueprint()->getCategory() == BuildingCategory::RESIDENTIAL) {
            std::string storageId = m_storageSystem->createStorage(StorageType::CHEST, "Colony");
            building->setStorageId(storageId);
            std::cout << "  Storage (Chest) created for house: " << storageId << std::endl;
        }
    }

    if (task->getBlueprint()->getId() == "door") {
        auto doorObj = std::make_shared<Door>("Door", task->getPosition(), task->getRotation(), m_colony);
        building->setDoor(doorObj);
        if (m_interactionSystem) {
            m_interactionSystem->registerInteractableObject(doorObj.get());
        }
    } else if (task->getBlueprint()->getId() == "bed") {
        auto bedObj = std::make_shared<Bed>("Bed", task->getPosition(), task->getRotation());
        building->setBed(bedObj);
        if (m_interactionSystem) {
            m_interactionSystem->registerInteractableObject(bedObj.get());
        }
...
void BuildingSystem::render() {
    for (const auto& building : m_buildings) {
        const auto& blueprint = m_blueprints[building->getBlueprintId()];
        if (blueprint) {
            if (!blueprint->getComponents().empty()) {
                for (const auto& comp : blueprint->getComponents()) {
                    ...
                    if (m_blueprints.count(comp.blueprintId)) {
                        const auto& compBp = m_blueprints.at(comp.blueprintId);
                        Model* model = compBp->getModel();
                        
                        Vector3 rotatedOffset = Vector3RotateByAxisAngle(comp.localPosition, { 0.0f, 1.0f, 0.0f }, building->getRotation() * DEG2RAD);
                        ...
                        if (model && model->meshCount > 0 && model->materialCount > 0) {
                            DrawModelEx(*model, pos, { 0.0f, 1.0f, 0.0f }, rot, { 1.0f, 1.0f, 1.0f }, color);
                        } else {
                            // Fallback rendering using CubeModel or Matrix Transforms
                            Color fallbackColor = color;
                            if (ColorEqual(color, WHITE)) fallbackColor = GRAY; 

                            Vector3 fallbackPos = pos;
                            fallbackPos.y += compBp->getSize().y / 2.0f;

                            if (cubeModelLoaded) {
                                DrawModelEx(cubeModel, fallbackPos, { 0.0f, 1.0f, 0.0f }, rot, compBp->getSize(), color);
                            } else {
                                ...
                                DrawCube(Vector3{0,0,0}, compBp->getSize().x, compBp->getSize().y, compBp->getSize().z, fallbackColor);
                                rlPopMatrix();
                            }
                            ...
                            if (comp.blueprintId == "wall" || isFloor(comp.blueprintId)) {
                                ...
                                rlTranslatef(fallbackPos.x, fallbackPos.y, fallbackPos.z);
                                rlRotatef(rot, 0.0f, 1.0f, 0.0f);
                                DrawCubeWires(Vector3{ 0.0f, 0.0f, 0.0f }, compBp->getSize().x, compBp->getSize().y, compBp->getSize().z, BLACK);
                                rlPopMatrix();
                            }
                        }
                    }
                ...
                if (model && model->meshCount > 0) {
                    DrawModelEx(*model, building->getPosition(), { 0.0f, 1.0f, 0.0f }, building->getRotation(), { 1.0f, 1.0f, 1.0f }, WHITE);
                } else {
    ...
    for (const auto& task : m_buildTasks) {
        ...
        DrawCube(Vector3Add(pos, {0, 2.5f, 0}), task->getProgress()/100.0f * 2.0f, 0.2f, 0.2f, GREEN);

        // Fallback rendering logic for tasks
        if (!blueprint->getComponents().empty()) {
             // Simplified task view - just bounds
             Vector3 size = blueprint->getSize();
             ...
             DrawCube(Vector3{0,0,0}, size.x, size.y, size.z, {200, 200, 200, 100});
             DrawCubeWires(Vector3{0,0,0}, size.x, size.y, size.z, GREEN);
             rlPopMatrix();
        } else if (blueprint->getModel() && blueprint->getModel()->meshCount > 0) {
             DrawModelEx(*blueprint->getModel(), pos, { 0.0f, 1.0f, 0.0f }, rotation, { 1.0f, 1.0f, 1.0f }, color);
        } else {
             Vector3 size = blueprint->getSize();
             Vector3 renderPos = pos;
             renderPos.y += size.y / 2.0f;
...
void BuildingSystem::renderStorageContents(BuildingInstance* building) {
    ...
    if (building->getStorageId().empty()) return;
    ...
    //           << " (" << building->getStorageId() << ") - Visual Slots: " << visualSlots.size() << std::endl;

    rlDisableBackfaceCulling();
    // Render visual slots stored in the building instance
    for (const auto& vSlot : visualSlots) {
        // Transform local position to world position
        Vector3 rotatedOffset = Vector3RotateByAxisAngle(vSlot.localPosition, { 0.0f, 1.0f, 0.0f }, building->getRotation() * DEG2RAD);
        ...
        if (building->getBlueprintId() == "stockpile" && terrain) {
            // For stockpiles, we ignore local Y relative to building and snap to terrain + offset
            // Use vSlot.localPosition.y as offset from ground
        ...
        switch (vSlot.type) {
            ...
            case Resources::ResourceType::Food: color = ORANGE; break;
            case Resources::ResourceType::Metal: color = LIGHTGRAY; break;
            case Resources::ResourceType::Gold: color = GOLD; break;
            case Resources::ResourceType::Water: color = BLUE; break;
            default: break;
        }
        ...
        DrawCube(Vector3{0,0,0}, vSlot.scale.x, vSlot.scale.y, vSlot.scale.z, color);
        // DrawCubeWires(Vector3{0,0,0}, vSlot.scale.x, vSlot.scale.y, vSlot.scale.z, YELLOW); // Less visual noise
        rlPopMatrix();
    }
    rlEnableBackfaceCulling();
}

void BuildingSystem::renderPreview(const std::string& blueprintId, Vector3 position, float rotation) {
    if (m_blueprints.find(blueprintId) == m_blueprints.end()) return;
    
    const auto& bp = m_blueprints.at(blueprintId);
...
bool BuildingSystem::getBuildingAtRay(Ray ray, BuildingInstance** outBuilding) const {
    ...
    if (hitBuilding) {
        if (outBuilding) *outBuilding = hitBuilding;
        return true;
    }
    return false;
}

std::vector<BuildingBlueprint*> BuildingSystem::getAvailableBlueprints() const {
...
std::vector<BuildingInstance*> BuildingSystem::getBuildingsInRange(Vector3 center, float radius) const {
...
BuildTask* BuildingSystem::getBuildTaskAt(Vector3 position, float radius) const {
    for (const auto& task : m_buildTasks) {
...
void BuildingSystem::enablePlanningMode(bool enable) {
    m_isPlanning = enable;
}
