...
struct StorageSlot {
  ...
  StorageSlot()
      : isOccupied(false), resourceType(Resources::ResourceType::Wood),
        amount(0) {}

  bool canAddResource(Resources::ResourceType type, int32_t amount,
...
public:
  ...
  struct StorageConfig {
    ...
    StorageConfig(StorageType t, const std::string &n,
    ...
    StorageConfig()
        : type(StorageType::INVENTORY), name("Default"), description(""),
          maxSlots(20), maxWeight(100.0f), maxVolume(100.0f), isStackable(true),
          stackLimitPerSlot(99), requiresAuthentication(false),
          accessTime(0.1f), isShared(false) {}
  };
  ...
  struct StorageInstance {
    ...
    std::unordered_map<Resources::ResourceType, int32_t>
        totalAmounts; // Cache dla szybkiego dostępu

    StorageInstance()
        : state(StorageState::EMPTY), currentWeight(0.0f), currentVolume(0.0f) {
    }
     ...
     */
    int32_t
    getMaxCapacityForResource(Resources::ResourceType type,
                              const Resources::Resource *resourceInfo) const;
  };
  ...
  public:
     ...
     */
    static std::unordered_map<StorageType, StorageConfig>
    createBaseStorageTypes() {
      std::unordered_map<StorageType, StorageConfig> storageTypes;

      // Ekwipunek gracza
      {
        StorageConfig inventory(StorageType::INVENTORY, "Ekwipunek",
                                "Podstawowy ekwipunek gracza");
      ...
      // Magazyn
      {
        StorageConfig warehouse(StorageType::WAREHOUSE, "Magazyn",
                                "Duży magazyn w budynkach");
      ...
      // Skrzynia
      {
        StorageConfig chest(StorageType::CHEST, "Skrzynia",
                            "Mała przenośna skrzynia");
        chest.maxSlots = 20;
      ...
      // Bank
      {
        StorageConfig bank(StorageType::BANK, "Bank",
                           "Bezpieczny bank dla cennych przedmiotów");
        bank.maxSlots = 200;
     ...
     */
    static StorageInstance createStorageInstance(StorageType type,
...
public:
  ...
  int32_t addResourceToStorage(const std::string &storageId,
                               const std::string &playerId,
                               Resources::ResourceType resourceType,
                               int32_t amount);
  int32_t removeResourceFromStorage(const std::string &storageId,
                                    const std::string &playerId,
                                    Resources::ResourceType resourceType,
                                    int32_t amount);

  int32_t getResourceAmount(const std::string &storageId,
                            Resources::ResourceType resourceType) const;
  std::unordered_map<Resources::ResourceType, int32_t>
  getAllResources(const std::string &storageId) const;
  bool canAddResource(const std::string &storageId,
                      Resources::ResourceType resourceType,
                      int32_t amount) const;

  // Missing method implementation
  bool addItemToStorage(const std::string &storageId, const Item &item,
                        int amount);
...
private:
  ...
  int32_t findSlotForResource(StorageInstance &storage,
                              Resources::ResourceType resourceType,
                              const Resources::Resource *resourceInfo) const;
  ...
  struct StorageCache {
    std::unordered_map<std::string, std::vector<Resources::ResourceType>>
        storageResourceTypes;
    std::chrono::high_resolution_clock::time_point lastCleanup;

    StorageCache() : lastCleanup(std::chrono::high_resolution_clock::now()) {}

    void addToCache(const std::string &storageId,
                    const std::vector<Resources::ResourceType> &types) {
      storageResourceTypes[storageId] = types;
    }

    bool getFromCache(const std::string &storageId,
                      std::vector<Resources::ResourceType> &types) const {
