...
void BuildingSystem::EnsureStorageForBuildingInstance(BuildingInstance* building) {
if (!building || !m_storageSystem) return;

// Check if it's a storage building and doesn't have storageId
const BuildingBlueprint* bp = building->getBlueprint();
if (!bp) return;

if (bp->getCategory() == BuildingCategory::STORAGE && building->getStorageId().empty()) {
    ...
    std::cout << "[BuildingSystem] DEBUG: Assigned storageId=" << storageId
              << " to building blueprint=" << building->getBlueprintId() << std::endl;
}
...
BuildTask::BuildTask(BuildingBlueprint* blueprint, Vector3 position, GameEntity* builder, float rotation)
    : m_blueprint(blueprint), m_position(position), m_builder(builder), m_rotation(rotation),
      m_progress(0.0f), m_state(BuildState::PLANNING), m_startTime(0.0f)
{
...
void BuildingSystem::ensureBlueprintExists(const std::string& blueprintId) {
    ...
    if (it != m_blueprints.end() && it->second) return;
...
void BuildingSystem::completeBuilding(BuildTask* task) {
    ...
    for (auto* worker : task->getWorkers()) {
        if (worker) worker->ClearBuildTask();
    }
