...
bool StorageSlot::canAddResource(Resources::ResourceType type, int32_t amountToCheck, const Resources::Resource* resourceInfo) const {
    if (!isOccupied) {
        return true;
    }
...
int32_t StorageSlot::getMaxCapacity(const Resources::Resource* resourceInfo) const {
    if (!resourceInfo) {
...
bool StorageSystem::StorageInstance::canAddResource(Resources::ResourceType type, int32_t amount, const Resources::Resource* resourceInfo) const {
...
int32_t StorageSystem::StorageInstance::getMaxCapacityForResource(Resources::ResourceType type, const Resources::Resource* resourceInfo) const {
...
void StorageSystem::StorageSlotPool::releaseSlot(std::unique_ptr<StorageSlot> slot) {
    if (slot) {
        slot->isOccupied = false;
        slot->amount = 0;
...
void StorageSystem::StorageSlotPool::clearPool() {
    getPool().clear();
}
...
void StorageSystem::initialize() {
    ...
    std::cout << "StorageSystem initialized with " << m_storageTypes.size() << " base types." << std::endl;
}

void StorageSystem::update(float deltaTime) {
    m_cache.cleanup();
}
...
const StorageSystem::StorageConfig* StorageSystem::getStorageConfig(StorageType type) const {
...
std::string StorageSystem::createStorage(StorageType type, const std::string& ownerId) {
    auto it = m_storageTypes.find(type);
    if (it == m_storageTypes.end()) {
        return "";
    }

    StorageInstance instance = StorageFactory::createStorageInstance(type, ownerId);
...
int32_t StorageSystem::addResourceToStorage(const std::string& storageId, const std::string& playerId,
    ...
    if (!config) {
        std::cout << "StorageSystem: Config not found for type " << (int)storage.type << std::endl;
        return 0;
    }

    if (config->requiresAuthentication && storage.ownerId != playerId && !config->isShared) {
        return 0;
    }
    ...
    std::cout << "[StorageSystem] Adding " << amount << " of type " << (int)resourceType
              << " to " << storageId << " by " << playerId << std::endl;
    ...
    if (config->isStackable) {
        for (size_t i = 0; i < storage.slots.size(); ++i) {
            ...
            if (slot.isOccupied && slot.resourceType == resourceType) {
                int32_t added = addResourceToSlot(storage, static_cast<int32_t>(i), resourceType, remaining, resourceInfo);
                remaining -= added;
                addedTotal += added;
            }
        }
    ...
    if (addedTotal > 0) {
        ...
        // notifyStorageEvent(event); // Disabled for now
    }

    if (addedTotal == 0) {
        // Failed to add resource (Full or no suitable slots)
        std::cout << "[StorageSystem] Failed to add any resource. Storage might be full or incompatible." << std::endl;
    } else {
        std::cout << "[StorageSystem] Successfully added " << addedTotal << " items." << std::endl;
    }
    return addedTotal;
}

int32_t StorageSystem::removeResourceFromStorage(const std::string& storageId, const std::string& playerId,
                                                 Resources::ResourceType resourceType, int32_t amount) {
    auto storageIt = m_storages.find(storageId);
    if (storageIt == m_storages.end()) {
        return 0;
...
int32_t StorageSystem::getResourceAmount(const std::string& storageId, Resources::ResourceType resourceType) const {
    auto it = m_storages.find(storageId);
...
std::unordered_map<Resources::ResourceType, int32_t> StorageSystem::getAllResources(const std::string& storageId) const {
    std::unordered_map<Resources::ResourceType, int32_t> resources;
    auto it = m_storages.find(storageId);
    if (it == m_storages.end()) return resources;
...
bool StorageSystem::canAddResource(const std::string& storageId, Resources::ResourceType resourceType, int32_t amount) const {
    auto it = m_storages.find(storageId);
...
void StorageSystem::addItemToStorage(const std::string& storageId, const Item& item, int amount) {
    // Map items to resource types based on name or type
    // This is a temporary solution until we fully integrate Item system with Resource system
    ...
    if (item.getItemType() == ItemType::RESOURCE) {
         if (name == "Wood") type = Resources::ResourceType::Wood;
         else if (name == "Stone") type = Resources::ResourceType::Stone;
         else if (name == "Metal") type = Resources::ResourceType::Metal;
         else if (name == "Gold") type = Resources::ResourceType::Gold;
         else if (name == "Food" || name == "Raw Meat") type = Resources::ResourceType::Food;
    } else if (item.getItemType() == ItemType::CONSUMABLE) {
         if (name == "Raw Meat" || name == "Cooked Meat" || name == "Berry") type = Resources::ResourceType::Food;
         else if (name == "Water") type = Resources::ResourceType::Water;
    }

    // Also check direct string mapping if enum check failed but name matches known resources
    if (type == Resources::ResourceType::None) {
        if (name == "Wood") type = Resources::ResourceType::Wood;
        else if (name == "Stone") type = Resources::ResourceType::Stone;
        else if (name == "Food") type = Resources::ResourceType::Food;
        else if (name == "Metal") type = Resources::ResourceType::Metal;
    }

    if (type != Resources::ResourceType::None) {
        addResourceToStorage(storageId, "system", type, amount);
    } else {
        std::cout << "StorageSystem: Cannot store item " << item.getDisplayName() << " (Type not mapped)" << std::endl;
    }
}

StorageSystem::StorageInstance* StorageSystem::getStorage(const std::string& storageId) const {
    auto it = m_storages.find(storageId);
    if (it != m_storages.end()) {
...
std::vector<std::string> StorageSystem::getPlayerStorages(const std::string& playerId) const {
...
bool StorageSystem::optimizeStorageLayout(const std::string& storageId) {
    ...
    const StorageConfig* config = getStorageConfig(storage.type);
    if (!config || !config->isStackable) return false;

    bool changed = false;
    
    for (size_t i = 0; i < storage.slots.size(); ++i) {
        if (!storage.slots[i].isOccupied) continue;
        ...
        for (size_t j = i + 1; j < storage.slots.size(); ++j) {
            if (storage.slots[j].isOccupied && 
                ...
                int32_t space = storage.slots[i].getMaxCapacity(nullptr) - storage.slots[i].amount;
                if (space > 0) {
...
StorageSystem::StorageSystemStats StorageSystem::getStats() const {
    ...
    stats.averageCapacityUsage = stats.totalStorages > 0 ? totalCapUsage / stats.totalStorages : 0.0f;
...
void StorageSystem::updateStorageState(StorageInstance& storage) {
    ...
    if (empty) storage.state = StorageState::EMPTY;
    else if (full) storage.state = StorageState::FULL;
    else storage.state = StorageState::PARTIAL;
...
int32_t StorageSystem::findSlotForResource(StorageInstance& storage, Resources::ResourceType resourceType, const Resources::Resource* resourceInfo) const {
    for (size_t i = 0; i < storage.slots.size(); ++i) {
        if (!storage.slots[i].isOccupied) return static_cast<int32_t>(i);
    }
    return -1;
}

int32_t StorageSystem::addResourceToSlot(StorageInstance& storage, int32_t slotIndex,
                                         Resources::ResourceType resourceType, int32_t amount, const Resources::Resource* resourceInfo) {
    if (slotIndex < 0 || slotIndex >= static_cast<int32_t>(storage.slots.size())) return 0;
    
    StorageSlot& slot = storage.slots[slotIndex];
    
    if (slot.isOccupied && slot.resourceType != resourceType) return 0;
    ...
    std::cout << "[StorageSystem] Slot " << slotIndex << ": Current=" << slot.amount
              << " Max=" << maxCap << " Adding=" << toAdd << std::endl;

    slot.amount += toAdd;
    return toAdd;
}

int32_t StorageSystem::removeResourceFromSlot(StorageInstance& storage, int32_t slotIndex, int32_t amount) {
    if (slotIndex < 0 || slotIndex >= static_cast<int32_t>(storage.slots.size())) return 0;
    
    StorageSlot& slot = storage.slots[slotIndex];
    if (!slot.isOccupied) return 0;
...
void StorageSystem::notifyStorageEvent(const std::any& event) {
    // Disabled
}