...
: m_colony(colony), m_buildingSystem(buildingSystem), m_timer(0.0f), m_updateInterval(0.5f), m_needsHousing(false)
...
if (!m_buildingSystem) return;
// Only assign jobs to idle settlers
for (auto* settler : settlers) {
    if (settler->GetState() != SettlerState::IDLE) continue;
    
    // Skip if hungry or tired (NeedsSystem should handle this, but extra check doesn't hurt)
    ...
    if (settler->performBuilding && !buildTasks.empty()) {
        ...
        for (auto* task : buildTasks) {
            // Limit workers per task to avoid crowding (e.g., 3 workers per building)
            if (task->getWorkerCount() >= 3) continue;
        ...
        if (bestTask) {
            ...
            // Manually increment worker count since Settler::AssignBuildTask might not do it immediately
            // or logic might be split. Ideally Settler::AssignBuildTask calls task->addWorker()
            // Let's assume we need to manage it here or verify if Settler does it.
            // Based on analysis, Settler::AssignBuildTask sets m_currentBuildTask.
            // We should probably let the task know, but `BuildTask` has `addWorker` method.
            // For safety, let's update it here if Settler doesn't.
            // However, if Settler calls it in Update, double counting might occur.
            // Let's verify BuildTask usage in Settler.cpp (it wasn't explicitly calling addWorker in reviewed snippet).
            // So we call it here to be safe and consistent with "centralized" logic.
            bestTask->addWorker(settler);
            std::cout << "ColonyAI: Assigned build task to " << settler->getName() << std::endl;
    ...
    if (settler->gatherWood && m_colony->isGatheringTaskActive("Wood")) {
        ...
        if (terrain) {
            ...
            if (bestTree) {
                ...
                settler->assignTask(TaskType::CHOP_TREE, bestTree, bestTree->getPosition());
                bestTree->reserve(settler->getName()); // Important: Reserve it!
                std::cout << "ColonyAI: Assigned chop tree task to " << settler->getName() << std::endl;
                continue; // Job assigned, move to next settler
            }
        }
    }
    ...
    if (settler->huntAnimals) {
        ...
        if (bestAnimal) {
            ...
            std::cout << "ColonyAI: Assigned hunting task to " << settler->getName() << std::endl;
            continue; // Job assigned, move to next settler
        }
    }
}
