...
struct ResourceRequirement {
  ...
  ResourceRequirement(std::string type, int amt, bool optional = false,
                      std::string desc = "")
      : resourceType(std::move(type)), amount(amt), isOptional(optional),
...
public:
  ...
  void addCost(Resources::ResourceType type, int amount) {
    m_requirements.push_back(
        ResourceRequirement(Resources::resourceTypeToString(type), amount));
  }
  ...
  void addComponent(const BlueprintComponent &comp) {
    m_components.push_back(comp);
  }
  const std::vector<BlueprintComponent> &getComponents() const {
    return m_components;
  }

  // Compatibility methods
  const std::vector<ResourceRequirement> &getRequirements() const {
  ...
  int getRequirementAmount(const std::string &resourceType) const {
