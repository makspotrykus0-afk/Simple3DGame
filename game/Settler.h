...
Settler(const std::string& name, const Vector3& pos, SettlerProfession profession);
virtual ~Settler();
...
void setProfession(SettlerProfession newProfession) { m_profession = newProfession; }
...
void clearPath() { m_currentPath.clear(); m_currentPathIndex = 0; }
...
void UpdateDepositing(float deltaTime, const std::vector<BuildingInstance*>& buildings);
void UpdatePickingUp(float deltaTime, std::vector<WorldItem>& worldItems, const std::vector<BuildingInstance*>& buildings);
void UpdateSearchingForFood(float deltaTime, const std::vector<Bush*>& bushes);
...
BuildingInstance* FindNearestWorkshop(const std::vector<BuildingInstance*>& buildings);
...
std::vector<std::string> m_ignoredStorages;// Job State History (to detect toggle)
...
Vector3 m_lastPosition = {0,0,0}; // ostatnia pozycja do wykrywania braku ruchu
float m_sleepCooldownTimer = 0.0f; // Blokada ponownego snu przez X sekund po obudzeniu
