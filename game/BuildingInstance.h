...
public:
  BuildingInstance(const std::string &blueprintId, const Vector3 &position,
                   float rotation)
      : GameEntity("Building_" + blueprintId), m_blueprintId(blueprintId),
        m_rotation(rotation), m_constructionProgress(0.0f), m_isBuilt(false),
        m_health(100.0f), m_maxHealth(100.0f), m_cachedBlueprint(nullptr) {
  ...
  const std::vector<VisualStorageSlot> &getVisualSlots() const {
    return m_visualSlots;
  }
  ...
  void updateVisualStorage(const std::vector<StorageSlot> &storageSlots) {
    // std::cout << "[DEBUG] BuildingInstance::updateVisualStorage - Input
    // slots: " << storageSlots.size() << std::endl;
    m_visualSlots.clear();
    ...
    for (const auto &slot : storageSlots) {
      if (slot.isOccupied && slot.amount > 0) {
        ...
        // Make items look slightly different based on type if needed, but scale
        // by amount primarily Using slightly larger base scale
    ...
    // std::cout << "[DEBUG] BuildingInstance::updateVisualStorage - Visual
    // slots created: " << m_visualSlots.size() << std::endl;
  }
  ...
  BoundingBox getBoundingBox() const {
    ...
    return {{pos.x - halfSize.x, pos.y, pos.z - halfSize.z},
  ...
  Door *getDoor() const { return m_door.get(); }
  ...
  void render() override {
    ...
    // Rendering storage slots is now handled by BuildingSystem to avoid
    // duplication and Z-fighting
  }
